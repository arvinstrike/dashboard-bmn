...
@section('content')
    <div class="content-wrapper">
        <div class="content">
            <div class="container">
                <h1>Edit Pengajuan RKBMN Bagian Non SBSK</h1>
                ...
                <form action="{{ route('pengajuan.reguler.update', $pengajuan->id) }}" method="POST"
                    id="formEditPengajuan">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                            <label for="tipe_pengajuan_display">Tipe Pengajuan</label>
                            <select name="tipe_pengajuan_display" id="tipe_pengajuan_display" class="form-control" required
                                readonly disabled>
                                <option value="revisi" {{ $pengajuan->tipe_pengajuan == 'revisi' ? 'selected' : '' }}>Revisi
                                    Anggaran Barang</option>
                                <option value="usulan" {{ $pengajuan->tipe_pengajuan == 'usulan' ? 'selected' : '' }}>Usulan
                                    Anggaran Barang</option>
                            </select>
                            <input type="hidden" name="tipe_pengajuan" id="tipe_pengajuan_hidden"
                    ...
                    <div class="form-group row">
                        <div class="col-sm-12">
                            <label for="keterangan_editable">Keterangan</label>
                            <textarea name="keterangan" id="keterangan" class="form-control" rows="4" placeholder="Masukkan keterangan..." required>{{ $pengajuan->keterangan }}</textarea>
                        </div>
                    </div>
                            ...
                            <select name="id_bagian_pelaksana" id="id_bagian_pelaksana" class="form-control" required>
                                ...
                                @foreach ($pelaksanaOptions as $option)
                                    <option value="{{ $option->id }}"
                                        {{ $pengajuan->id_bagian_pelaksana == $option->id ? 'selected' : '' }}>
                                        {{ $option->uraianbagian }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="table-responsive-custom">
                        ...
                        <table class="table table-bordered table-hover" id="tabelBarang">
                            ...
                            <tbody>
                                @foreach ($detilItems as $index => $item)
                                    <tr class="barang-row" data-index="{{ $index }}">
                                        <td class="nomor text-center">{{ $index + 1 }}</td>
                                        <td class="col-barang">
                                            <select name="barang[{{ $index }}][kode_barang]"
                                                class="form-control dropdown-barang">
                                                <option value="{{ $item['kode_barang'] }}" selected>
                                                    {{ $item['ur_sskel'] }}
                                                </option>
                                            </select>
                                        </td>
                                        <td class="col-keterangan">
                                            <textarea name="barang[{{ $index }}][keterangan_barang]" class="form-control keterangan-input" rows="2"
                                                placeholder="Masukkan keterangan barang...">{{ $item['keterangan_barang'] }}</textarea>
                                        </td>
                                        <td class="col-qty">
                                            <input type="number" name="barang[{{ $index }}][kuantitas]"
                                                class="form-control kuantitas-input text-center" min="1"
                                                value="{{ $item['kuantitas'] }}">
                                        </td>
                                        <td class="col-harga">
                                            <input type="text" name="barang[{{ $index }}][harga]"
                                                class="form-control harga-input text-end"
                                                data-raw-value="{{ $item['harga'] }}"
                                                value="{{ number_format($item['harga'], 0, ',', '.') }}" placeholder="0">
                                        </td>
                                        <td class="col-total">
                                            <input type="text" name="barang[{{ $index }}][total]"
                                                class="form-control total-input text-end bg-light"
    ...
    <style>
        ...
        .keterangan-input {
            ...
            /* Fixed height sama dengan input lain */
            min-height: 38px !important;
            /* Minimum height tetap */
            max-height: 120px;
            /* Maximum height untuk resize */
            font-size: 0.85rem;
            padding: 6px 8px;
            ...
            overflow-y: auto;
            /* Scroll vertikal jika teks terlalu panjang */
            box-sizing: border-box;
            /* Include padding dalam perhitungan height */
        }
        ...
        #tabelBarang {
            margin-bottom: 0;
            min-width: 1200px;
            /* Minimum width untuk memastikan semua kolom terlihat dengan baik */
            white-space: nowrap;
        }
        ...
        #tabelBarang .keterangan-input {
            height: 38px !important;
            /* Konsisten dengan form-control lain */
            min-height: 38px !important;
            font-size: 0.85rem;
            padding: 6px 8px;
            border: 1px solid #ced4da;
        ...
        @media (max-width: 768px) {
            ...
            #tabelBarang .form-control {
                height: 38px;
                /* Tinggi standar untuk semua input */
                padding: 6px 8px;
                font-size: 0.9rem;
                border: 1px solid #ced4da;
                border-radius: 0.25rem;
        ...
        /* Styling untuk grup pemeliharaan */
        .select2-results__group {
            padding: 8px 12px;
            font-weight: bold;
            color: #495057;
    ...
    <script>
        $(document).ready(function() {
            ...
            const config = {
                ...
                // Item statis belanja pemeliharaan
                staticPemeliharaan: [
                    {
                    ...
                    {
                        id: 'Pemeliharaan Gedung dan Bangunan lainya',
                        text: 'Pemeliharaan Gedung dan Bangunan lainya',
                        category: 'belanjaPemeliharaan'
                    },
                    {
                    ...
                    {
                        id: 'Pemeliharaan Peralatan dan Mesin lainya',
                        text: 'Pemeliharaan Peralatan dan Mesin lainya',
                        category: 'belanjaPemeliharaan'
                    },
                    {
                ...
                // Item statis belanja pemeliharaan persediaan
                staticPemeliharaanPersediaan: [
                    ...
                    {
                        id: 'Persediaan Pemeliharaan lainya',
                        text: 'Persediaan Pemeliharaan lainya',
                        category: 'belanjaPemeliharaanPersediaan'
                    }
                ]
            };
            ...
            function adjustTableLayout() {
                const $table = $('#tabelBarang');
                const $headerCells = $table.find('thead th');
                const $bodyCells = $table.find('tbody tr:first td');

                $headerCells.each(function(index) {
                    // Sesuaikan lebar header dengan body cell
                    $(this).width($bodyCells.eq(index).width());
                });
            }

            $(window).on('resize load', adjustTableLayout);

            $(document).on('input', '.kuantitas-input, .harga-input', function() {
                const $input = $(this);

                if ($input.hasClass('kuantitas-input')) {
                    // Validasi kuantitas
                    let value = parseInt($input.val()) || 1;
                    value = Math.max(1, value); // Minimal 1
                    $input.val(value);
                }

                if ($input.hasClass('harga-input')) {
                    // Validasi harga dengan format
                    const rawValue = utils.parseNumber($input.val());
                    $input.val(utils.formatNumber(rawValue));
        ...
        `;
            ...
            function initializeSelect2(selector) {
                ...
                selector.each(function() {
                    ...
                    // Prepare data dengan grouping - UPDATED dengan 3 kategori
                    const groupedData = [
                        {
            ...
            function fetchBarangOptions() {
                ...
                return $.ajax({
                    ...
                    success: function(data) {
                        ...
                        // Gabungkan dengan item statis (UPDATED dengan 2 array)
                        allBarangOptions = [
                        ...
                        console.log(`Loaded ${config.staticPemeliharaan.length} pemeliharaan items, ${config.staticPemeliharaanPersediaan.length} persediaan items, and ${dbOptions.length} database items`);
                    },
                    error: function(xhr, status, error) {
                        console.error("Error fetching barang options:", error);

                        // Jika gagal fetch dari database, gunakan item statis saja - UPDATED
                        allBarangOptions = [
                            ...config.staticPemeliharaan,
                            ...config.staticPemeliharaanPersediaan
                        ];
            ...
            function calculateRowTotal($row) {
                ...
                // Update total display dengan format yang konsisten
                $row.find('.total-input')
                    .val(utils.formatNumber(total))
                    .attr('title', `Total: ${utils.formatNumber(total)}`); // Tambahkan tooltip
            }
            ...
            // Add row - DIPERBAIKI: Tidak otomatis membuka dropdown
            $('#tambahBaris').on('click', function() {
                ...
                // DIHAPUS: .select2('open') agar dropdown tidak terbuka otomatis
                // $newRow.find('.dropdown-barang').select2('open');

                // Optional: Scroll ke baris baru atau focus ke dropdown tanpa membuka
                $newRow.find('.dropdown-barang').focus();
            });
            ...
            $('#formEditPengajuan').on('submit', function(e) {
                ...
                $('#tabelBarang .barang-row').each(function() {
                    ...
                    const harga = utils.parseNumber($row.find('.harga-input').attr(
                        'data-raw-value') || '0');
                    ...
                    if (kuantitas <= 0) {
                        errorMessages.push(
                            `Baris ${$row.find('.nomor').text()}: Kuantitas harus lebih dari 0.`
                        );
                        isValid = false;
                    }
                    ...
                    if (harga <= 0) {
                        errorMessages.push(
                            `Baris ${$row.find('.nomor').text()}: Harga harus lebih dari 0.`);
                        isValid = false;
                    }
                ...
                if (!hasValidRow) {
                    errorMessages.push(
                        'Minimal harus ada satu baris barang yang lengkap (barang dipilih, kuantitas > 0, dan harga > 0).'
                    );
                    isValid = false;
                }

                if (!isValid) {
                    utils.showAlert('error', 'Validasi Error', errorMessages.join('\n'));
                    return;
                }
            ...
            // Updated submitForm function for edit
            function submitForm() {
                ...
                const formData = new FormData($('#formEditPengajuan')[0]);

                // Restore formatted values immediately
                $('.harga-input').each(function(index) {
                ...
                $.ajax({
                    url: $('#formEditPengajuan').attr('action'),
                    type: 'POST', // Laravel uses POST for PUT/PATCH requests when including _method
                    data: formData,
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    success: function(response) {
                        ...
                        if (response.status === 'berhasil') {
                            Swal.fire({
                                title: 'Sukses',
                                text: response.message || 'Data pengajuan berhasil diupdate.',
                                icon: 'success',
                                confirmButtonText: 'OK'
                            }).then(() => {
                            ...
                            let errorMessage = response.message || 'Data pengajuan gagal diupdate.';
                            if (response.errors) {
            ...
            function init() {
                console.log("Initializing edit form with static pemeliharaan items...");

                // Set default value for tahun_anggaran
                $('#tahun_anggaran').trigger('change');

                // Fetch barang options (includes static items)
                fetchBarangOptions();
                ...
                // Log configuration for debugging - UPDATED
                console.log("Static belanja pemeliharaan items:", config.staticPemeliharaan);
                console.log("Static belanja pemeliharaan persediaan items:", config.staticPemeliharaanPersediaan);
            }
