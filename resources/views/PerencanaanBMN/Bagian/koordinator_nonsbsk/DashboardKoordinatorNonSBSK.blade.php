{{--resources/views/PerencanaanBMN/Bagian/koordinator_nonsbsk/DashboardPageReguler.blade.php--}}
@extends('layouts.app')
...
<div class="content-wrapper">
    ...
    <div class="content">
        <div class="container">
            <h1>[Koordinator Pengadaan] Dashboard Pengajuan RKBMN Non SBSK</h1>
                    ...
                    <table id="pengajuanTable" class="table table-bordered table-striped">
                        <thead>
                            ...
                            <tr>
                                ...
                                <th><input type="text" class="form-control form-control-sm filter-input" data-column="1" placeholder="Filter nilai..."></th>
                                <th>
                                ...
                                <th><input type="text" class="form-control form-control-sm filter-input" data-column="3" placeholder="Filter bagian pengusul..."></th>
                                <th><input type="text" class="form-control form-control-sm filter-input" data-column="4" placeholder="Filter bagian pelaksana..."></th>
                        ...
                        <tbody>
                            @forelse($pengajuan as $p)
                                <tr>
                                    <td>
                                        {{ $p->kode_pengajuan ?? ($p->tipe_pengajuan == 'revisi'
                                            ? "PMB-{$p->id_bagian_pengusul}-{$tahunanggaran}-".str_pad($p->id, 5, '0', STR_PAD_LEFT)
                                            : "PRC-{$p->id_bagian_pengusul}-".($tahunanggaran+1)."-".str_pad($p->id, 5, '0', STR_PAD_LEFT))
                                        }}
                                    </td>
                                    <td>
                                        @php
                                            $totalNilai = 0;
                                            if($p->tipe_pengajuan == 'revisi') {
                                                $totalNilai = $p->detilRevisi->sum('total');
                                            } else {
                                                $totalNilai = $p->detilPengajuan->sum('total');
                                            }
                                        @endphp
                                    ...
                                    <td>
                                        <div class="text-truncate" style="max-width: 150px;" title="{{ $p->bagianPelaksana->uraianbagian ?? 'Unknown' }}">
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            <a href="{{ route('koordinator.review', $p->id) }}" class="btn btn-info btn-sm mr-1" title="Review Pengajuan">
                                                <i class="fas fa-eye mr-1"></i>Review
                                            </a>
                                            @if($status == 'Disetujui' && ($p->berita_acara_koordinator_signed_path))
                                                <a href="{{ route('koordinator.downloadBeritaAcara', $p->id) }}" class="btn btn-success btn-sm" title="Download Berita Acara">
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center py-4">
                                        ...
                                        <small class="text-muted">Pengajuan akan muncul ketika ada pengajuan baru dari pelaksana</small>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            for (let j = 0; j < cells.length - 1; j++) { // Skip the last column (Actions)
                if (filters[j] && filters[j] !== '') {
                    ...
                    if (j === 2) { // Status column - get text from badge
